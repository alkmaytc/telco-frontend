  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#fbf9f8', color: '#041632', display: 'flex', flexDirection: 'column', fontFamily: 'Hanken Grotesk, sans-serif' }}>
      ...
      <header className="responsive-header" style={{ borderBottom: '2px solid #041632', padding: '16px 24px', backgroundColor: '#ffffff' }}>
        ...
        <button onClick={fetchOrderHistory} disabled={!user || error} className="refresh-btn" style={{ display: 'flex', alignItems: 'center', gap: '8px', border: '2px solid #041632', backgroundColor: (user && !error) ? '#ffffff' : '#ccc', padding: '8px 16px', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px', fontWeight: '700', cursor: (user && !error) ? 'pointer' : 'not-allowed', boxShadow: (user && !error) ? '2px 2px 0px 0px #041632' : 'none' }}>
          ...
          <div>
            ...
            {!user ? (
              ...
              <div style={{ border: '2px solid red', backgroundColor: '#fff5f5', padding: '40px 24px', textAlign: 'center', fontFamily: 'JetBrains Mono, monospace', marginTop: '20px', boxShadow: '4px 4px 0px 0px red' }}>
                ...
                <button
                  onClick={() => navigate('/')}
                  style={{ backgroundColor: 'red', color: '#ffffff', border: '2px solid red', padding: '10px 20px', fontSize: '12px', fontWeight: '700', cursor: 'pointer', fontFamily: 'JetBrains Mono, monospace' }}
                >
                  ANA SAYFAYA GÜVENLİ DÖNÜŞ
                </button>
              </div>
            ) : (
            ...
            <div style={{ flex: 1, fontFamily: 'JetBrains Mono, monospace', fontSize: '11px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '6px', lineHeight: '1.4' }}>
              {!user || error ? (
                <div style={{ color: 'red', fontStyle: 'italic' }}>
                  [SECURITY DISCONNECT] SESSION BREAK OR REJECTED BY ACCESS CONTROL POLICY...
                </div>
              ) : (
                <>
                  {history.map((logItem, index) => (
